...
def test_get_settings_set_DCI_CERT_FILE_to_default_if_not_setted():
    ...
    assert settings["dci_cert_file"] == "/etc/dci-downloader/ssl/dci.pem"
...
def test_get_settings_use_DCI_CERT_FILE_from_env():
    settings = get_settings(
        ...
        env_variables={
            ...
            "DCI_API_SECRET": "",
            "DCI_CS_URL": "",
            "DCI_CERT_FILE": "/etc/dci-rhel-agent/ssl/dci.pem",
        },
    )
    assert settings["dci_cert_file"] == "/etc/dci-rhel-agent/ssl/dci.pem"
