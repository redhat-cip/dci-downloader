...
@mock.patch("dci_downloader.main.downloader")
def test_download_components_with_tags(
    downloader_mock, get_topic_mock, get_components_per_topic_mock
):
    ...
    download_components(
        {"name": "RHEL-8", "download_folder": "/var/www/html", "tags": ["tag1", "tag2"]}
    )
    get_components_per_topic_mock.assert_called_with(
        ...
        sort="-created_at",
        topic_id="t1",
        where="type:compose,state:active,tags:tag1,tags:tag2",
    )
    download_components(
        {"name": "RHEL-8", "download_folder": "/var/www/html", "tags": []}
    )
    get_components_per_topic_mock.assert_called_with(
        limit=1,
        offset=0,
    ...
    download_components(
        {"name": "RHEL-8", "download_folder": "/var/www/html", "tags": ["tag1"]}
    )
    get_components_per_topic_mock.assert_called_with(
        limit=1,
        offset=0,
        sort="-created_at",
        topic_id="t1",
        where="type:compose,state:active,tags:tag1",
    )
