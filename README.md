...
dci-downloader downloads SSL certificates prior the download phase. To customize the CRT and KEY path you can change `` and `` variables.

If you are using `settings.yml` file change

```
download_key_file: /etc/dci-rhel-agent/dci.key
download_crt_file: /etc/dci-rhel-agent/dci.crt
```
