...
def get_files_list(base_url, settings):
    ...
    r = requests.get(files_list_url, verify=settings["dci_cert_file"])
    r.raise_for_status()
    return r.json()
...
@retry()
def download_file(file, settings):
    r = requests.get(file["source"], stream=True, verify=settings["dci_cert_file"])
    r.raise_for_status()
