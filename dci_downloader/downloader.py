...
def download_file(url, file_path, cert, key):
    print("Downloading %s into %s" % (url, file_path))
...
def download(base_url, files_list, local_repo_path, cert, key):
    ...
    for file in get_files_to_download(local_repo_path, files_list):
        local_dir = os.path.join(local_repo_path, file["path"])
        mkdir_p(local_dir)
        local_file_path = os.path.join(local_dir, file["name"])
        file_url = (
            "%s/%s/%s" % (base_url, file["path"], file["name"])
            if file["path"]
            else "%s/%s" % (base_url, file["name"])
        )
