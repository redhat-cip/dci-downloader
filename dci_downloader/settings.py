...
def _clean_topics(topics, download_folder):
    new_topics = []
    for topic in topics:
        new_topic = {"download_folder": download_folder}
        new_topic.update(_clean_topic(topic))
...
def get_settings(sys_args, env_variables):
    ...
    cleaned_topics = _clean_topics(settings["topics"], download_folder)
    settings.update({"topics": cleaned_topics})
    return settings
